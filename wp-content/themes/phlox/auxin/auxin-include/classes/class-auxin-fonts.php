...
class Auxin_Fonts {
   ...
   */
    ...
    public function admin_init(){
        // $this->parse_typography();
        // axpp( auxin_options() );
        // axpp( $this->selected_fonts );

        $this->default_fonts = array(
            'auxin_body_font' => '_gof_Raleway:regular'
        );
    }
    ...
    function get_fonts_list(){
        ...
        // var tag = document.createElement('script'); tag.src = "//ajax.googleapis.com/ajax/libs/jquery/1.11.1/jquery.min.js"; document.getElementsByTagName('head')[0].appendChild( tag );
        // jQuery('.content > div').each( function( i, el ){
        //     var $el   = jQuery( el );
        //     var $h2   = $el.find('h2');
        //     var name  = $h2.attr('name');
        //     var title = $h2.text();
        //     var url   = $el.find('pre').eq(0).text();
        //     console.log( "array( 'name' => '"+name+"'     , 'title' => '"+title+"'   , 'thickness' => '400,700' , 'url' => '"+url+"' )," );
        // });
     ...
     */
    function get_special_charecters(){
     ...
     * @return void
     */
    function update_special_charecters(){
        update_option( THEME_ID. '_font_subsets', $this->get_special_charecters() );
    }
     ...
     * @return void
     */
    function extract_font( $font_info, $option_id = '' ){
        $components = array();
     ...
     * @return void
     */
    function create_typography_options( $option_id, $components, $wrap_by_quote = false ){
     ...
     * @return void
    */
    function extract_components( $font_info, $prefix = '', $category = 'others' ){
        ...
        $selected_font = array( 'face' => $face, 'thickness' => $thickness, 'link_query' => $link_query, 'url' => $url );
     ...
     */
    function get_font_url( $face, $link_query, $category = 'others' ){
        ...
        foreach ( $this->selected_fonts as $group_fonts_id => $group_fonts ) {
            foreach ( $group_fonts as $font_info ) {
                ...
                if( 'google' == $group_fonts_id ){
                    ...
                    //$google_fonts_queries .= $font_info['link_query'] ? $font_info['link_query']. '|' : '';
    ...
    */
    function parse_typography(){
        ...
        $this->selected_fonts = array(
            'google'  => array(
                'Raleway' => array(
                    'face'       => 'Raleway',
                    'thickness'  => '300',
                    'link_query' => 'Raleway:300',
                    'url'        => '//fonts.googleapis.com/css?family=Raleway:300'
                )
            ),
            'others'  => array()
        );
        ...
        update_option( THEME_ID. '_font_urls'  , $this->enqueue_font_urls );
