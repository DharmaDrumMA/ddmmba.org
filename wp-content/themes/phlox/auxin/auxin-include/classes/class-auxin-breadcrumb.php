...
class Auxin_Breadcrumb {
  ...
  function __construct() {
    ...
    public function render() {
        ...
        $crumbs .=  $this->create_crumb( _x( 'Home', 'Home in breadcrumb' , 'phlox'), home_url(), null );
        ...
        if( is_single() ) {
            ...
            if( $this_page_type == "news" ) {
                ...
                $crumbs  .= $this->create_crumb( $blog_title, get_permalink( $blog_page_id ) );
                $crumbs  .= $this->get_single_breadcrumb_branch( $tax );
                ...
                if( $nav_type == 'breadcrumb' ){
                    ...
                    $list_page = sprintf( '<a href="%s">%s</a>', get_post_type_archive_link( get_post_type() ), __( 'show all', 'phlox' ) );
    ...
    public function create_crumb( $title = '', $link = '', $sep = '' ){
        if( $title ) {
            ...
            $trimmed_title = auxin_get_trimmed_string( $title, 30, '...' );


            if( $link ){
                return sprintf( '%s<span><a href="%s" title="%s">%s</a></span>', $sep , $link, esc_attr( $title ), $trimmed_title );
            } else {
    ...
    public function get_next_prev_post_link( $start_tag = '', $end_tag = '', $sep = '', $middle_tag = '' ){
        ...
        $output  = $start_tag;
        $output .= get_previous_post_link( '%link', _x( 'previous', 'navigate to previous page', 'phlox') );
        $output .= $sep;
        $output .= ! empty( $middle_tag ) ? $middle_tag . $sep : '';
        $output .= get_next_post_link( '%link', _x( 'next', 'navigate to next page', 'phlox') );
        $output .= $end_tag;
