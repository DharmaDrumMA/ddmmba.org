...
class Auxin_Import {
    ...
    public function admin_init() {
        ...
        register_importer( 'auxin-importer',
            __( 'Auxin Importer', 'phlox' ),
            sprintf( __( 'Import demo data for %s theme.', 'phlox' ), '<strong>' . THEME_NAME_I18N . '</strong>' ),
    ...
    function header() {
        echo '<div class="wrap">';
        screen_icon();
        echo '<h2>' . __( 'Importing Demo Data', 'wordpress-importer' ) . '</h2><br />';
        ...
        if( ! empty( $_GET['demo-id'] ) ) {
            ...
            if( ! empty( $demo_list[ $demo_id ]['file'] ) ){
                ...
                if ( ! class_exists( 'WP_Importer' ) ) {
                    ...
                    printf( __( 'In order to import the demo data, you need to have "WordPress Importer" plugin installed. Please %s install and activate %s it and then try importing the demo data again.', 'phlox' ),
                        '<a href="'. $this->welcome_page_url .'&tab=plugins">',
                        '</a>'
                    );
                ...
                printf( __( 'The demo that you have requested is not valid. Please try to %s select a demo %s to import.', 'phlox' ),
            ...
            printf( __( 'Please %s select a demo %s to import.', 'phlox' ), '<a href="'. $this->demo_page_url .'">', '</a>' );
        }
