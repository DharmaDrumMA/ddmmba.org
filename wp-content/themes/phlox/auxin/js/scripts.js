...
;(function($, window, document, undefined){
    ...
    $(function(){
                    ...
                    if( undefined !== control.params.dependencies && control.params.dependencies.length ){
                        ...
                        for ( var i = 0, l = control.params.dependencies.length; i < l; i++ ) {
                            ...
                            if( undefined === dependControl ){
                                console.log('The dependency ID "' + depend.id + '" was not found.');
                                continue;
                            }
        ...
        if( wp.customize ){
            wp.customize.controlConstructor['auxin_textarea'] = wp.customize.AuxinControl.extend({
                ...
                ready: function() {
                    ...
                    var control = this,
                        picker  = this.container.find( 'textarea' );
        ...
        if( wp.customize ){
            ...
            wp.customize.controlConstructor['auxin_code'] = wp.customize.AuxinControl.extend({
                ...
                ready: function() {
                    ...
                    var control = this,
                        picker  = this.container.find( editor_picker);
        ...
        var spectrum_picker = '.mini-color-wrapper input[type="text"]',
            spectrum_args = {
                allowEmpty: true,
                showInput: true,
                showAlpha: true,
        ...
        if( wp.customize ){
            wp.customize.controlConstructor['auxin_color'] = wp.customize.AuxinControl.extend({
                ...
                ready: function() {
                    ...
                    var control = this,
                        picker  = this.container.find( spectrum_picker );

                    //spectrum_args.change      = null;
                    //spectrum_args.showButtons = false;
                    picker.val( control.setting() ).spectrum( spectrum_args );

                    picker.on("dragstop.spectrum", function(e, color){
                        if( color === null) {
                            picker.val('');
                        } else {
                            picker.val( color.toString() );
                        }
                        control.setting.set( picker.val() );
                        // refresh the preview
        ...
        if( wp.customize ){
            wp.customize.controlConstructor['auxin_radio_image'] = wp.customize.AuxinControl.extend({
                ...
                ready: function() {
                    ...
                    var control = this,
                        picker  = this.container.find( visual_select_picker );


                    picker.val( control.setting() ).avertaVisualSelect( visual_select_args );
        ...
        if( wp.customize ){
            wp.customize.controlConstructor['auxin_switch'] = wp.customize.AuxinControl.extend({
                ...
                ready: function() {
                    ...
                    var control = this,
                        picker  = this.container.find( switchery_picker );

                    picker.val( control.setting() );

                    switchery_args.size = 'small';

                    var switchery = new Switchery( picker[0], switchery_args );

                    picker.on( 'change', 'input:checkbox', function() {
                        control.setting.set( picker.val() );
                        // refresh the preview
        ...
        if( wp.customize ){
            wp.customize.controlConstructor['auxin_typography'] = wp.customize.AuxinControl.extend({
                ...
                ready: function() {
                    ...
                    var control = this,
                        picker  = this.container.find( font_select_picker );

                    picker.val( control.setting() ).avertaFontSelector( font_select_args );
        ...
        if( wp.customize ){
            wp.customize.controlConstructor['auxin_media'] = wp.customize.AuxinControl.extend({
                ...
                ready: function() {
                    ...
                    var control = this,
                        picker  = this.container.find( attach_media_picker );

                    if( control.params.attachments ) {
                        attach_media_args.srcMap = control.params.attachments
                    }

                    picker.val( control.setting() ).avertaAttachMedia( attach_media_args );
        ...
        if( wp.customize ){
            wp.customize.controlConstructor['auxin_base'] = wp.customize.AuxinControl.extend({
                ...
                ready: function() {
                    ...
                    var control = this,
                        picker  = this.container.find( 'input[type]' );
            ...
            if( wp.customize ){
                wp.customize.controlConstructor['auxin_select2'] = wp.customize.AuxinControl.extend({
                    ...
                    ready: function() {
                        ...
                        var control = this,
                            picker  = this.container.find( select2_picker );

                        picker.val( control.setting() ).select2( select2_args );
            ...
            if( wp.customize ){
                wp.customize.controlConstructor['auxin_select2_multiple'] = wp.customize.AuxinControl.extend({
                    ...
                    ready: function() {
                        ...
                        var control = this,
                            picker  = this.container.find( select2_picker_multi );

                        picker.val( control.setting() ).select2( select2_args_multi );
    ...
    */
    ...
    p.applyPresets = function( presetDataStacks, selectedStack ){
        ...
        for( var presetTargetId in presetTargets ){
            ...
            // console.log( "Looking for field : " + presetTargetId );
